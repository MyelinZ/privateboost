...
class Aggregator:
    ...
    def __init__(
        ...
        if len(shareholders) < threshold:
            raise ValueError(f"Need at least {threshold} shareholders, got {len(shareholders)}")
    ...
    def _select_shareholders(
        ...
        sh_commitments = [(sh, get_commitments(sh)) for sh in self._shareholders]
    ...
    def define_bins(self) -> List[BinConfiguration]:
        ...
        selected_shs, commitments = self._select_shareholders(lambda sh: sh.get_stats_commitments())
        ...
        target_mean = float(self._means[-1])
        target_mean = np.clip(target_mean, 1e-7, 1 - 1e-7)
        self._model.initial_prediction = float(np.log(target_mean / (1 - target_mean)))
    ...
    def compute_splits(
        ...
        for node_id in active_nodes:
            shares = self._collect_gradient_shares(selected_shs, depth, commitments, node_id)
            ...
            self._node_totals[node_id] = NodeTotals(gradient_sum=total_g, hessian_sum=total_h)
    ...
    def finish_round(self) -> None:
        ...
        def compute_leaf_value(node_id: int) -> Leaf:
            ...
            return Leaf(value=value, n_samples=0)
