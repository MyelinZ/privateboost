...
#[test]
fn test_path_hiding_model_identical_to_baseline() {
    // Decoy submissions are shares of zero: they must leave every reconstructed
    // aggregate exactly unchanged, so training with and without path hiding on
    // identical data must produce bit-identical models. Reconstruction is exact
    // field arithmetic, so this holds regardless of the sharing randomness.
    let dataset = read_csv(Path::new("tests/data/heart_disease.csv"), "target").unwrap();
    let n_trees = 5;
    let max_depth = 3;

    let train = |hide_path: bool| -> Vec<f64> {
        let shareholders: Vec<ShareHolder> =
            (0..3).map(|i| ShareHolder::new(i, (i + 1) as u64, 10)).collect();
        let mut aggregator =
            Aggregator::new(shareholders, 10, 2, 10, 0.15, 2.0, BinMethod::Gaussian).unwrap();
        let mut clients: Vec<Client> = dataset
            .features
            .iter()
            .zip(dataset.targets.iter())
            .enumerate()
            .map(|(idx, (feats, &target))| {
                Client::new(format!("client_{}", idx), feats.clone(), target, 3, 2, None)
            })
            .collect();
        ...
        for _ in 0..n_trees {
            for _ in 0..max_depth {
                ...
                for client in &mut clients {
                    let shares = client
                        .compute_gradient_shares(&ctx, &Loss::Logistic, hide_path)
                        .unwrap();
                    for share in shares {
                        let sh_idx = share.share.x as usize - 1;
                        aggregator.shareholders_mut()[sh_idx].receive_gradients(share);
        ...
        aggregator.model().predict(&dataset.features)
    };

    let base = train(false);
    let hidden = train(true);
    assert_eq!(base.len(), hidden.len());
    for (i, (a, b)) in base.iter().zip(hidden.iter()).enumerate() {
        assert!(
            a == b,
            "prediction {i} differs: {a} (no hiding) vs {b} (hiding); \
             decoy shares must not perturb aggregates"
        );
    }
}

#[test]
fn test_gradient_message_independent_of_bin_position() {
    // The wire format must not reveal which bin a client's value falls into:
    // the message carries no (feature, bin) tags, the share vector densely
    // spans every (feature, bin) position, and the serialized length is
    // identical regardless of which position is occupied.
    let bins = vec![
        BinConfiguration {
            feature_idx: 0,
            edges: vec![f64::NEG_INFINITY, 0.0, 1.0, 2.0, 3.0],
        },
        BinConfiguration {
            feature_idx: 1,
            edges: vec![f64::NEG_INFINITY, 0.0, 1.0, 2.0, 3.0],
    ...
    // Client A falls in the lowest bin of every feature, client B in the highest.
    let mut client_a = Client::new("a".into(), vec![-5.0, -5.0], 1.0, 3, 2, Some(1));
    let mut client_b = Client::new("b".into(), vec![99.0, 99.0], 0.0, 3, 2, Some(2));

    let shares_a = client_a
        .compute_gradient_shares(&ctx, &Loss::Logistic, false)
        .unwrap();
    let shares_b = client_b
        .compute_gradient_shares(&ctx, &Loss::Logistic, false)
        .unwrap();
    ...
    for (a, b) in shares_a.iter().zip(shares_b.iter()) {
        assert_eq!(
            ...
            "message length must not depend on the occupied bin"
        );
        // Dense span: 2 values (g, h) x 2 features x 5 bins.
        assert_eq!(a.share.values.len(), 2 * 2 * 5);
        assert_eq!(b.share.values.len(), 2 * 2 * 5);
    }
}
