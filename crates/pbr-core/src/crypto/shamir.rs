//! Shamir sharing over the Mersenne field.
//!
//! `share` splits a whole vector at once, one polynomial per element with a
//! shared evaluation point per party: party i holds every element's value at
//! x = i + 1, so a share is a vector, not a scalar. Any `threshold` shares
//! reconstruct; fewer reveal nothing.
//!
//! Shares are additively homomorphic at a fixed x, which is what lets a
//! shareholder sum many clients' shares (`Share::add`) and return one
//! aggregate that reconstructs to the sum of their secrets, having seen none
//! of them.
