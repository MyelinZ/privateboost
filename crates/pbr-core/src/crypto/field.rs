//! The Mersenne prime field the protocol shares over.
//!
//! p = 2^61 - 1 is chosen so reduction is an add and a conditional subtract
//! (see `reduce`) rather than a division, and so a product of two elements
//! fits in u128 with room to fold the high bits back. Every value on the wire
//! is a canonical residue, strictly below p.
...
impl Distribution<MersenneField> for StandardUniform {
    fn sample<R: rand::Rng + ?Sized>(&self, rng: &mut R) -> MersenneField {
        // Rejection sampling over 61-bit values: exactly uniform on [0, PRIME).
        // A plain `u64 % PRIME` would over-weight residues 0..7, since
        // 2^64 = 8 * PRIME + 8.
        loop {
            let x = rng.random::<u64>() >> 3;
            if x != PRIME {
                return MersenneField(x);
            }
        }
    }
}
...
mod tests {
    ...
    fn test_from_i64_min() {
        ...
        let expected = MersenneField(PRIME - ((1u64 << 63) % PRIME));
