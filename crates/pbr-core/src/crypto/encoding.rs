//! Fixed-point encoding between f64 and field elements.
//!
//! A value is scaled by `PRECISION` and rounded to an integer, so the field
//! carries about 7 decimal digits after the point and sums stay exact as long
//! as the accumulated magnitude stays under p/2: above that a sum wraps and
//! decodes as a large negative number rather than saturating. Negative values
//! occupy the top half of the field, which is why `decode` goes through
//! `to_i64` instead of reading the residue directly.
