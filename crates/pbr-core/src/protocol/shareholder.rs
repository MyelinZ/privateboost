//! One shareholder's share pool.
//!
//! Holds one evaluation point's share of every client's committed vector and
//! answers only aggregate queries: the commitments it holds, and sums over a
//! caller-supplied subset of them. It never exposes an individual share.
//!
//! `min_clients` is the anonymity floor and is enforced here rather than by
//! the caller: a sum over fewer than that many commitments is refused, so no
//! aggregate can be narrowed to a single contributor by asking for a small
//! enough subset. It counts commitments, meaning clients, not records, since
//! one client folds its whole batch into one commitment.
...
/// Read-side interface the aggregator needs from a shareholder. Implemented
/// by the in-process `ShareHolder` and, in deployments, by RPC adapters to
/// remote shareholders. Receiving shares is not part of this trait: remote
/// shareholders receive over their own transport.
pub trait ShareSource {
    ...
    fn gradients_sum(&self, depth: usize, commitments: &[Commitment], node_id: usize)
        -> Result<Share>;
}

impl ShareSource for ShareHolder {
    fn stats_commitments(&self) -> BTreeSet<Commitment> {
        ShareHolder::get_stats_commitments(self)
    }
    fn gradient_commitments(&self, depth: usize) -> BTreeSet<Commitment> {
        ShareHolder::get_gradient_commitments(self, depth)
    }
    fn gradient_node_ids(&self, depth: usize) -> BTreeSet<usize> {
        ShareHolder::get_gradient_node_ids(self, depth)
    }
    fn stats_sum(&self, commitments: &[Commitment]) -> Result<Share> {
        ShareHolder::get_stats_sum(self, commitments)
    }
    fn gradients_sum(&self, depth: usize, commitments: &[Commitment], node_id: usize)
        -> Result<Share> {
        ShareHolder::get_gradients_sum(self, depth, commitments, node_id)
    }
}
