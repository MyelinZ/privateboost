...
\author{Bernhard~Specht,~\IEEEmembership{}
        Samaher~Garbaya,
        Orhan~Ermis,
        Reinhard~Schneider,
        Ricardo~Chavarriaga,
        Djamel~Khadraoui,
        and~Zied~Tayeb%
\thanks{B.~Specht is with MyelinZ and the University of Luxembourg, Esch-sur-Alzette, Luxembourg (e-mail: bernhard.specht@myelinz.com).}%
\thanks{S.~Garbaya and Z.~Tayeb are with MyelinZ. Z.~Tayeb is also with the University of Lincoln, Lincoln, United Kingdom.}%
\thanks{O.~Ermis and D.~Khadraoui are with the Luxembourg Institute of Science and Technology (LIST), Esch-sur-Alzette, Luxembourg.}%
\thanks{R.~Schneider is with the Luxembourg Centre for Systems Biomedicine (LCSB), Esch-sur-Alzette, Luxembourg.}%
\thanks{R.~Chavarriaga is with ZHAW Zurich University of Applied Sciences, Winterthur, Switzerland.}}

\markboth{IEEE Transactions on Information Forensics and Security}%
{Specht \MakeLowercase{\textit{et al.}}: PrivateBoost}
...
\begin{IEEEkeywords}
Federated learning, gradient boosting, secret sharing, privacy-preserving machine learning, cross-device learning
\end{IEEEkeywords}
...
We consider $c$ clients, each holding a single labeled sample $(x_i, y_i)$. A set of $n$ \textit{shareholders} (e.g., $n = 3$) act as intermediate aggregation points. A single \textit{aggregator} coordinates the training process. Fig.~\ref{fig:architecture} illustrates the overall system architecture.
...
On Heart Disease, mean retention is $\bar{R} = 98.1\%$ across 13 features, with 10 features achieving $R_f = 100\%$ (Fig.~\ref{fig:gain_retention}). Histogram bins are constructed using aggregated statistics from the first protocol round, requiring no additional privacy cost. Features with skewed distributions may have suboptimal bin placement, with most data concentrated in a few bins. This explains the 90--93\% retention observed on features like \texttt{oldpeak}.
